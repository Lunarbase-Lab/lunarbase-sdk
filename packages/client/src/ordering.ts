...
export class CursorReorderBuffer {
  ...
  len(): number { return this.pending.size; }
  isEmpty(): boolean { return this.pending.size === 0; }
  ...
  drainAll(): ChainUpdate[] { return this.drain(() => true); }
...
function cursorKey(cursor: ChainCursor, rank: number): CursorKey { return [cursor.blockNumber, cursor.transactionIndex ?? 0n, cursor.logIndex ?? 0n, rank]; }
function updateKey(update: ChainUpdate): CursorKey { return update.kind === "Head" ? cursorKey(update.cursor, 0) : update.kind === "Log" ? cursorKey(update.log.cursor, 1) : update.kind === "Reorg" ? cursorKey(update.newHead, 2) : update.kind === "Gap" ? (update.cursor ? cursorKey(update.cursor, 3) : [2n ** 64n - 1n, 0n, 0n, 3]) : [0n, 0n, 0n, 4]; }
function watermarkKey(cursor: ChainCursor): CursorKey { return cursor.transactionIndex === undefined && cursor.logIndex === undefined ? [cursor.blockNumber, 2n ** 32n - 1n, 2n ** 32n - 1n, 255] : [cursor.blockNumber, cursor.transactionIndex ?? 0n, cursor.logIndex ?? 0n, 255]; }
function compareKey(left: CursorKey, right: CursorKey): number { for (let index = 0; index < left.length; index += 1) { if (left[index] < right[index]) return -1; if (left[index] > right[index]) return 1; } return 0; }
