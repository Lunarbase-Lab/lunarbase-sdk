...
import { MonadExecutionNormalizer } from "./core.js";
import {
  BoundedFrameQueue,
    ...
    try {
      ...
      while (!signal?.aborted) {
        ...
        try {
          ...
          yield gap(`Monad sidecar failed; canonical recovery required: ${message(error)}`);
          return;
        }
        if (frame === undefined) {
          yield gap("Monad sidecar closed; canonical recovery required");
          return;
        }
        ...
        try {
          ...
          yield gap(`invalid Monad sidecar JSON: ${message(error)}`);
          return;
        }
        if (value.error) {
          yield gap(`Monad sidecar subscription error: ${JSON.stringify(value.error)}`);
          return;
        }
        ...
        if (type === "alert") {
          ...
          if (isRecoveryAlert(detail)) {
            yield normalizer.normalizeGap(detail);
            return;
          }
          continue;
        }
        if (type === "health") {
          if (result.stalled === true) {
            yield normalizer.normalizeGap("Monad parser reports stalled reader");
            return;
          }
          continue;
        }
        ...
        if (typeof subscription !== "string") {
          yield gap("Monad sidecar notification is missing subscription id");
          return;
        }
        if (subscription === allSubscription && (type === "newHead" || type === "blockStart")) {
          ...
          try {
            ...
            yield gap(`invalid Monad sidecar head: ${message(error)}`);
            return;
          }
        ...
        if (subscription === logsSubscription && type === "log" && result.kind === "event") {
          ...
          try {
            ...
            yield gap(`invalid Monad sidecar log: ${message(error)}`);
            return;
          }
          if (log.address.toLowerCase() !== filter.address.toLowerCase()) continue;
          const normalized = normalizer.normalizeTxnLog({
