...
import { BaseFlashblocksNormalizer, type FlashblockHeader, type FlashblockLog } from "./normalizer.js";

/** Resource bounds for Base Flashblocks transport frames and queues. */
export interface BaseFlashblocksConfig {
  readonly maxFrameBytes: number;
  readonly reorderCapacity: number;
  readonly queueCapacity: number;
}
export const DEFAULT_BASE_FLASHBLOCKS_CONFIG: BaseFlashblocksConfig = Object.freeze({
...
export class BaseFlashblocksBackend implements NormalizedBackend {
  ...
  /** Creates a Base Flashblocks backend with injectable WebSocket transport. */
  constructor(
    ...
    config: Partial<BaseFlashblocksConfig> = {},
  ...
  /** Consumes pendingLogs/newFlashblocks and emits normalized provisional updates. */
  subscribe(network: Network, filter: ContractFilter, signal?: AbortSignal): AsyncIterable<ChainUpdate> {
    if (network !== this.network || network !== NetworkValue.Base)
      throw new RpcError("INVALID", "Base Flashblocks backend network mismatch");
  ...
  private async *readSocket(
    ...
    const onMessage = (event: SocketEvent) => {
      ...
      if (frame === undefined) queue.fail(new Error("Base Flashblocks delivered a non-text frame"));
      else if (new TextEncoder().encode(frame).byteLength > this.config.maxFrameBytes)
        queue.fail(new Error("Base Flashblocks frame exceeded configured bound"));
      else queue.push(frame);
    };
    const onError = (event: SocketEvent) =>
      queue.fail(event.error instanceof Error ? event.error : new Error("Base Flashblocks WebSocket error"));
    ...
    try {
      ...
      socket.send(
        JSON.stringify({
          ...
          params: ["pendingLogs", filterOptions(filter)],
        }),
      );
      ...
      const reorder = new CursorReorderBuffer(this.config.reorderCapacity);
      while (!signal?.aborted) {
        let frame: string | undefined;
        try {
          frame = await queue.next(signal);
        } catch (error) {
          yield gap(`Base Flashblocks failed; canonical recovery required: ${message(error)}`);
          return;
        }
        if (frame === undefined) {
          yield gap("Base Flashblocks closed; canonical recovery required");
          return;
        }
        ...
        try {
          ...
          yield gap(`invalid Base Flashblocks JSON: ${message(error)}`);
          return;
        }
        if (value.error) {
          yield gap(`Base Flashblocks subscription error: ${JSON.stringify(value.error)}`);
          return;
        }
        if (value.id !== undefined && typeof value.result === "string") {
          if (Number(value.id) === 1) pendingSubscription = value.result;
          if (Number(value.id) === 2) flashblocksSubscription = value.result;
          continue;
        }
        ...
        if (!subscription || result === undefined) {
          yield gap("Base Flashblocks notification is missing subscription/result");
          return;
        }
        if (subscription === flashblocksSubscription) {
          let payloadId: string;
          try {
            payloadId = parsePayloadId(result);
          } catch (error) {
            yield gap(`invalid Base Flashblock payload: ${message(error)}`);
            return;
          }
          const previous = [...headers.values()]
            .flat()
            .find((header) => header.payloadId.toLowerCase() === payloadId.toLowerCase())?.blockNumber;
          let header: FlashblockHeader;
          try {
            header = parseFlashblockHeader(result, previous);
          } catch (error) {
            yield gap(`invalid Base Flashblock payload: ${message(error)}`);
            return;
          }
          const previousBlock = headers.keys().next().value as bigint | undefined;
          if (previousBlock !== undefined && header.blockNumber > previousBlock + 1n) {
            yield gap("Base Flashblocks skipped one or more block payloads; canonical recovery required");
            return;
          }
          const items = headers.get(header.blockNumber) ?? [];
          items.push(header);
          if (items.length > 128) items.shift();
          headers.set(header.blockNumber, items);
          while (headers.size > 64) headers.delete(headers.keys().next().value as bigint);
          const head = normalizer.normalizeHeader(header);
          if (head) {
            try {
              reorder.push(head);
            } catch (error) {
              yield gap(`Base Flashblocks reorder failed: ${message(error)}`);
              return;
            }
            for (const update of reorder.drainThrough(headerCursor(header, this.chainId))) yield update;
          }
          continue;
        }
        if (subscription === pendingSubscription) {
          let log: ContractLog;
          try {
            log = parseRpcLog(result, this.chainId, CommitmentValue.Realtime);
          } catch (error) {
            yield gap(`invalid Base pending log: ${message(error)}`);
            return;
          }
          ...
          if (!header) {
            yield {
              kind: "Gap",
              cursor: log.cursor,
              reason: "pending log arrived without a matching Flashblock header",
            };
            return;
          }
          const flashblockLog: FlashblockLog = {
            header,
          ...
          try {
            updates = normalizer.normalizeLog(flashblockLog);
          } catch (error) {
            yield gap(`Base Flashblocks normalization failed: ${message(error)}`);
            return;
          }
          try {
            for (const update of updates) reorder.push(update);
          } catch (error) {
            yield gap(`Base Flashblocks reorder failed: ${message(error)}`);
            return;
          }
          const latest = headers.get(log.cursor.blockNumber)?.at(-1);
          if (latest) for (const update of reorder.drainThrough(headerCursor(latest, this.chainId))) yield update;
        }
      }
    } finally {
...
function validateConfig(config: BaseFlashblocksConfig): BaseFlashblocksConfig {
  for (const [name, value] of Object.entries(config))
    if (!Number.isSafeInteger(value) || value <= 0) throw new Error(`Base Flashblocks ${name} must be positive`);
  return Object.freeze(config);
}
function filterOptions(filter: ContractFilter): Record<string, unknown> {
...
function parsePayloadId(value: unknown): string {
  if (!value || typeof value !== "object") throw new RpcError("INVALID", "Flashblock result is not an object");
  const payload = (value as Record<string, unknown>).payload_id;
  if (typeof payload !== "string" || !/^0x[0-9a-f]+$/i.test(payload) || payload.length > 66 || payload.length % 2 !== 0)
    throw new RpcError("INVALID", "invalid Flashblock payload_id");
...
function parseFlashblockHeader(value: unknown, previousBlockNumber?: bigint): FlashblockHeader {
  ...
  const blockNumber =
    base && typeof base === "object"
      ? parseHexU64((base as Record<string, unknown>).block_number, "Flashblock base.block_number")
      : previousBlockNumber;
...
function headerCursor(header: FlashblockHeader, chainId: bigint): ChainCursor {
  return {
    chainId,
...
function selectHeader(headers: FlashblockHeader[] | undefined, blockHash?: string): FlashblockHeader | undefined {
  ...
  return (
    (blockHash
      ? headers
          .slice()
          .reverse()
          .find((header) => header.blockHash?.toLowerCase() === blockHash.toLowerCase())
      : undefined) ?? headers.at(-1)
  );
}
...
function gap(reason: string): ChainUpdate {
