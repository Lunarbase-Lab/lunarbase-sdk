use lunarbase_client::{Commitment, MonadHead, MonadTxnLog, SourceError};
...
fn parse_head(value: &Value, block_start: bool) -> Result<MonadHead, ParserProtocolError> {
    Ok(MonadHead {
...
fn parse_log(value: &Value) -> Result<MonadTxnLog, ParserProtocolError> {
    let topics = value
        .get("topics")
